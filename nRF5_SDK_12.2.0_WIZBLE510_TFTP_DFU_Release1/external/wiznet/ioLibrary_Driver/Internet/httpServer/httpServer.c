...
#define NRF_LOG_MODULE_NAME "HTTP"
#include "nrf_log.h"
...
void httpServer_run(uint8_t seqnum)
{
	...
	switch(getSn_SR(s))
	{
		case SOCK_ESTABLISHED:
            NRF_LOG_RAW_INFO("SOCK_ESTABLISHED\r\n");
			// Interrupt clear
				...
				case STATE_HTTP_IDLE :
					if ((len = getSn_RX_RSR(s)) > 0)
					{
						...
						*(((uint8_t *)http_request) + len) = '\0';
						...
						NRF_LOG_RAW_INFO("\r\n");
						NRF_LOG_RAW_INFO("> HTTPSocket[%d] : HTTP Request received ", s);
						NRF_LOG_RAW_INFO("from %d.%d.%d.%d : %d\r\n", destip[0], destip[1], destip[2], destip[3], destport);
						NRF_LOG_RAW_INFO("> HTTPSocket[%d] : [State] STATE_HTTP_REQ_DONE\r\n", s);
#endif
						...
						while(getSn_TX_FSR(s) != (getSn_TXBUF_SIZE(s)*1024))
						{
							...
							{
#ifdef _HTTPSERVER_DEBUG_
								NRF_LOG_RAW_INFO("> HTTPSocket[%d] : [State] STATE_HTTP_REQ_DONE: TX Buffer clear timeout\r\n", s);
#endif
								break;
							}
						}
...
#ifdef _HTTPSERVER_DEBUG_
					NRF_LOG_RAW_INFO("> HTTPSocket[%d] : [State] STATE_HTTP_RES_INPROC\r\n", s);
#endif
...
#ifdef _HTTPSERVER_DEBUG_
					NRF_LOG_RAW_INFO("> HTTPSocket[%d] : [State] STATE_HTTP_RES_DONE\r\n", s);
#endif
...
//#endif
...
#ifdef _HTTPSERVER_DEBUG_
            NRF_LOG_RAW_INFO("> HTTPSocket[%d] : ClOSE_WAIT\r\n", s);	// if a peer requests to close the current connection
#endif
			disconnect(s);
			break;
		...
		case SOCK_CLOSED:
#ifdef _HTTPSERVER_DEBUG_
			NRF_LOG_RAW_INFO("> HTTPSocket[%d] : CLOSED\r\n", s);
#endif
			if(socket(s, Sn_MR_TCP, HTTP_SERVER_PORT, 0x00) == s)    /* Reinitialize the socket */
			{
#ifdef _HTTPSERVER_DEBUG_
				NRF_LOG_RAW_INFO("> HTTPSocket[%d] : OPEN\r\n", s);
#endif
			}
			break;
...
static void send_http_response_header(uint8_t s, uint8_t content_type, uint32_t body_len, uint16_t http_status)
{
	switch(http_status)
	{
		case STATUS_OK: 		// HTTP/1.1 200 OK
			...
			{
#ifdef _HTTPSERVER_DEBUG_
			NRF_LOG_RAW_INFO("> HTTPSocket[%d] : HTTP Response Header - STATUS_OK\r\n", s);
#endif
			...
			{
#ifdef _HTTPSERVER_DEBUG_
			NRF_LOG_RAW_INFO("> HTTPSocket[%d] : HTTP Response Header - NONE / CGI or XML\r\n", s);
#endif
...
#ifdef _HTTPSERVER_DEBUG_
			NRF_LOG_RAW_INFO("> HTTPSocket[%d] : HTTP Response Header - STATUS_BAD_REQ\r\n", s);
#endif
			memcpy(http_response, ERROR_REQUEST_PAGE, sizeof(ERROR_REQUEST_PAGE));
			break;
		case STATUS_NOT_FOUND:	// HTTP/1.1 404 Not Found
#ifdef _HTTPSERVER_DEBUG_
			NRF_LOG_RAW_INFO("> HTTPSocket[%d] : HTTP Response Header - STATUS_NOT_FOUND\r\n", s);
#endif
	...
	{
#ifdef _HTTPSERVER_DEBUG_
		NRF_LOG_RAW_INFO("> HTTPSocket[%d] : [Send] HTTP Response Header [ %d ]byte\r\n", s, (uint16_t)strlen((char *)http_response));
#endif
...
#ifdef _HTTPSERVER_DEBUG_
			NRF_LOG_RAW_INFO("> HTTPSocket[%d] : HTTP Response body - file name [ %s ]\r\n", s, (uint32_t)HTTPSock_Status[get_seqnum].file_name);
#endif
/////////////////////////////////////////////////////////////////////////////////////////////////

#ifdef _HTTPSERVER_DEBUG_
			NRF_LOG_RAW_INFO("> HTTPSocket[%d] : HTTP Response body - file len [ %ld ]byte\r\n", s, file_len);
#endif
		}
		else
		{
...
#ifdef _HTTPSERVER_DEBUG_
			NRF_LOG_RAW_INFO("> HTTPSocket[%d] : HTTP Response end - file len [ %ld ]byte\r\n", s, send_len);
#endif
		}
#ifdef _USE_FLASH_
...
#ifdef _HTTPSERVER_DEBUG_
			NRF_LOG_RAW_INFO("> HTTPSocket[%d] : HTTP Response body - send len [ %ld ]byte\r\n", s, send_len);
#endif
	}
...
/*****************************************************/
	...
	else if(HTTPSock_Status[get_seqnum]->storage_type == SDCARD)
	{
		...
		if(fr != FR_OK)
		{
			send_len = 0;
#ifdef _HTTPSERVER_DEBUG_
		NRF_LOG_RAW_INFO("> HTTPSocket[%d] : [FatFs] Error code return: %d (File Read) / HTTP Send Failed - %s\r\n", s, fr, HTTPSock_Status[get_seqnum].file_name);
#endif
		}
		else
		{
...
#ifdef _HTTPSERVER_DEBUG_
	NRF_LOG_RAW_INFO("> HTTPSocket[%d] : [Send] HTTP Response body [ %ld ]byte\r\n", s, send_len);
#endif

    NRF_LOG_RAW_INFO("> HTTPSocket[%d] : %s [ %ld ]byte\r\n", s, (uint32_t)buf, send_len);
	...
	else
	{
		HTTPSock_Status[get_seqnum].file_offset += send_len;
#ifdef _HTTPSERVER_DEBUG_
		NRF_LOG_RAW_INFO("> HTTPSocket[%d] : HTTP Response body - offset [ %ld ]\r\n", s, HTTPSock_Status[get_seqnum].file_offset);
#endif
	}
...
#ifdef _HTTPSERVER_DEBUG_
	NRF_LOG_RAW_INFO("> HTTPSocket[%d] : HTTP Response Header + Body - CGI\r\n", s);
#endif
	send_len = sprintf((char *)buf, "%s%d\r\n\r\n%s", RES_CGIHEAD_OK, file_len, http_body);
#ifdef _HTTPSERVER_DEBUG_
	NRF_LOG_RAW_INFO("> HTTPSocket[%d] : HTTP Response Header + Body - send len [ %d ]byte\r\n", s, send_len);
#endif
...
static void http_process_handler(uint8_t s, st_http_request * p_http_request)
{
	...
	switch (p_http_request->METHOD)
	{
		...
		case METHOD_GET :
			...
			NRF_LOG_RAW_INFO("> HTTPSocket[%d] : Request Type = %d\r\n", s, p_http_request->TYPE);
			NRF_LOG_RAW_INFO("> HTTPSocket[%d] : Request URI = %s\r\n", s, (uint32_t)uri_name);
#endif

			if(p_http_request->TYPE == PTYPE_CGI)
			{
			NRF_LOG_RAW_INFO("\r\n> HTTPSocket[%d] : PTYPE_CGI\r\n", s);
			...
			else
			{
				...
				{
#ifdef _HTTPSERVER_DEBUG_
					NRF_LOG_RAW_INFO("> HTTPSocket[%d] : Unknown Page Request\r\n", s);
#endif
				...
				{
#ifdef _HTTPSERVER_DEBUG_
					NRF_LOG_RAW_INFO("> HTTPSocket[%d] : Find Content [%s] ok - Start [%ld] len [ %ld ]byte\r\n", s, (uint32_t)uri_name, content_addr, file_len);
#endif
				...
				{
#ifdef _HTTPSERVER_DEBUG_
					NRF_LOG_RAW_INFO("> HTTPSocket[%d] : Requested content len = [ %ld ]byte\r\n", s, file_len);
#endif
		...
		case METHOD_POST :
			...
			NRF_LOG_RAW_INFO("> HTTPSocket[%d] : Request URI = %s ", s, (uint32_t)uri_name);
...
#ifdef _HTTPSERVER_DEBUG_
//				NRF_LOG_RAW_INFO("> HTTPSocket[%d] : [CGI: %s] / Response len [ %ld ]byte\r\n", s, content_found?"Content found":"Content not found", file_len);
#endif
				if(content_found && (file_len <= (DATA_BUF_SIZE-(strlen(RES_CGIHEAD_OK)+8))))
				{
...
uint8_t display_reg_webContent_list(void)
{
	...
	{
		NRF_LOG_RAW_INFO(">> Web content file not found\r\n");
		ret = 0;
	}
	else
	{
		...
		{
			NRF_LOG_RAW_INFO(" [%d] ", i+1);
			NRF_LOG_RAW_INFO("%s, ", (uint32_t)web_content[i].content_name);
			NRF_LOG_RAW_INFO("%ld byte, \r\n", web_content[i].content_len);
			if (web_content[i].content_len < 30)
            {
                NRF_LOG_RAW_INFO("[%s]\r\n", (uint32_t)web_content[i].content);
            }
			else 
            {
                NRF_LOG_RAW_INFO("...\r\n");
