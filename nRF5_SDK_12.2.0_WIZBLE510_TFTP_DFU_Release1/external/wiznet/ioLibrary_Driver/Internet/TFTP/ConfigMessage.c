...
static void process_firmware_upload_init(uint8_t sock, Remote_Info *remote_info, WIZnet_Header *wiznet_header)
{
	...
	//write_eeprom(0, &firmware_upload_info, sizeof(Firmware_Upload_Info)); //hoon
...
void reply_firmware_upload_done(uint8_t sock)
{
	...
	//read_eeprom(0, &firmware_upload_info, sizeof(Firmware_Upload_Info));  //hoon
	...
	ptr = 0;
	memcpy(buffer, (uint8_t *)&firmware_upload_info.wiznet_header, sizeof(WIZnet_Header));
	ptr += sizeof(WIZnet_Header);
	...
	memcpy(ip, (uint8_t *)firmware_upload_info.configtool_info.ip, sizeof(uint8_t) * 4);
	memcpy(&port, (uint16_t *)&firmware_upload_info.configtool_info.port, sizeof(uint16_t));
...
void do_udp_config(uint8_t sock)
{
	...
	case SOCK_UDP:
		...
		if (len >= sizeof(WIZnet_Header)) {
			recvfrom(sock, (uint8_t *)&wiznet_header, sizeof(WIZnet_Header), remote_info.ip, (uint16_t *)&remote_info.port);
