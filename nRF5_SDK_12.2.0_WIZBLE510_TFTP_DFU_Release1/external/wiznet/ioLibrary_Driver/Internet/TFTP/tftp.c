...
static void recv_tftp_data(uint8_t *msg, uint32_t msg_len)
{
	...
	switch(get_tftp_state())
	{
...
#ifdef F_STORAGE
        if (nrf_dfu_flash_erase((uint32_t*)MAIN_APPLICATION_START_ADDR, CEIL_DIV(BOOTLOADER_START_ADDR-MAIN_APPLICATION_START_ADDR, CODE_PAGE_SIZE), NULL) != FS_SUCCESS)
        {
        ...
        {
            nrf_delay_ms(10);
            if ((nrf_dfu_flash_store((uint32_t *)(MAIN_APPLICATION_START_ADDR+(((data->block_num-1)/8) *  CODE_PAGE_SIZE)), (uint32_t *)write_data, CEIL_DIV(CODE_PAGE_SIZE, sizeof(uint32_t)), NULL)) != FS_SUCCESS)
            {
